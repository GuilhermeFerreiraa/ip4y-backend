...
class Person extends \Faker\Provider\Person
{
     ...
     * @param string|null $birthdate (1800-2099) 'Y-m-d', 'Y-m', 'Y'  I.E. '1981-06-16', '2085-03', '1900'
     ...
     */
    public function cnp($gender = null, $birthdate = null, $county = null, $isResident = true)
    {
        ...
        $date = $this->getbirthdate($birthdate);
    ...
    {
        if (empty($birthdate)) {
            $birthdateParts = [self::numberBetween(1800, 2099)];
        } else {
            $birthdateParts = explode('-', $birthdate);
        }
        $baseDate = \Faker\Provider\DateTime::dateTimeBetween("first day of January {$birthdateParts[0]}", "last day of December {$birthdateParts[0]}");

        switch (count($birthdateParts)) {
            case 1:
                $birthdateParts[] = $baseDate->format('m');
                //don't break, we need the day also
                // no break
            case 2:
                $birthdateParts[] = $baseDate->format('d');
        ...
        if ($birthdateParts[0] < 1800 || $birthdateParts[0] > 2099) {
            throw new \InvalidArgumentException("Invalid date of birth - year must be between 1800 and 2099, '{$birthdateParts[0]}' received");
        }

        $birthdateFinal = implode('-', $birthdateParts);
        $date = \DateTime::createFromFormat('Y-m-d', $birthdateFinal);
        //a full (invalid) date might have been supplied, check if it converts
        if ($date->format('Y-m-d') !== $birthdateFinal) {
            throw new \InvalidArgumentException("Invalid date of birth - '{$date->format('Y-m-d')}' generated based on '{$birthdate}' received");
        }
     ...
     * @return int
     */
    protected static function getGenderDigit(\DateTime $birthdate, $gender, $isResident)
    {
        if (!$isResident) {
            return 9;
        }
